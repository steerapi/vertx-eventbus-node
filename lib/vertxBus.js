...
SockJS = require("sockjs-client-ws");
...
vertx.EventBus = function(url, options) {
  ...
  var that = this;
  var sockJSConn = new SockJS.create(url, options);
  sockJSConn.on("connection", function(){
    sockJSConn.onopen();
  });
  sockJSConn.on("data", function(message){
    sockJSConn.onmessage({data:message});
  });
  sockJSConn.on("close", function(){
    sockJSConn.onclose();    
  });
